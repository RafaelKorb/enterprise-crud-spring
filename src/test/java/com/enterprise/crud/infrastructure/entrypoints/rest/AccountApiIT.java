...
class AccountApiIT {
    ...
    void accountLifecycle() throws Exception {
        ...
        assertThat(post("/accounts/" + id + "/credits", "{\"amount\": 100.00}")).hasStatusOk();
        assertThat(post("/accounts/" + id + "/debits", "{\"amount\": 30.25}"))
                .hasStatusOk()
                .bodyJson()
        ...
        assertThat(post("/accounts/" + id + "/debits", "{\"amount\": 100.00}"))
                .hasStatus(HttpStatus.UNPROCESSABLE_CONTENT);
        assertThat(post("/accounts/" + id + "/debits", "{\"amount\": 0.001}"))
                .hasStatus(HttpStatus.UNPROCESSABLE_CONTENT);

        assertThat(put("/accounts/" + id + "/status", "{\"status\": \"BLOCKED\"}"))
                .hasStatusOk()
                .bodyJson()
                .hasPathSatisfying("$.status", status -> status.assertThat().isEqualTo("BLOCKED"));
        assertThat(post("/accounts/" + id + "/credits", "{\"amount\": 1.00}")).hasStatus(HttpStatus.CONFLICT);
    ...
    void duplicateDocumentIsConflict() throws Exception {
        ...
        assertThat(post("/accounts", "{\"documentNumber\": \"52998224725\"}")).hasStatus(HttpStatus.CONFLICT);
    }

    @Test
    void invalidDocumentIsUnprocessable() {
        assertThat(post("/accounts", "{\"documentNumber\": \"123\"}")).hasStatus(HttpStatus.UNPROCESSABLE_CONTENT);
    }

    @Test
    void unknownAccountIsNotFound() {
        assertThat(mvc.get().uri("/accounts/0198a3c0-0000-7000-8000-000000000099"))
    ...
    void cursorWalksEveryAccountExactlyOnce() throws Exception {
        ...
        String cursor = null;
        do {
            String uri = cursor == null ? "/accounts?limit=2" : "/accounts?limit=2&cursor=" + cursor;
    ...
    private String open(String documentNumber) throws Exception {
        MvcTestResult result = post("/accounts", "{\"documentNumber\": \"%s\"}".formatted(documentNumber)).exchange();
